...
**Последнее обновление:** 2026-06-18 (**+3 фото-работы Tommy Cash** — adidas / Maison Margiela / снегоход, категория `set-props`, между видео; «смешанный ряд» в `WorkGrid`: компактная фото-полоса 3-в-ряд внутри крупной 2-up сетки; мобильный — 1 колонка; теперь 15 работ; импорт через `import-use-this.ts`, мёрж в манифест. Прежнее: редизайн шапки + автоплей видео; A–G + H2)
